...
刺激：用户要求查看连通域，设置紧密度域值

响应：系统计算忽略小于域值之后的边的图的连通域，按照顶点数量，输出每个域的顶点总数

刺激：用户输入非法的紧密度域值（小于0）

响应：系统提示紧密度域值错误，要求用户重新输入

#### 3.2.3.3 相关功能需求

| 编号                        | 用例描述                       |
| --------------------------- | ------------------------------ |
| DomainNum                   | 系统允许用户查看连通域         |
| DomainNum.setClosenessMin   | 系统允许用户设置紧密度域值     |
| DomainNum.checkClosenessMin | 系统需要判断紧密度域值是否合法 |
